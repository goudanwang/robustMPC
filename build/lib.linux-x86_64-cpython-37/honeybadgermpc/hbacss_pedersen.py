...
class Hbacss0:
        ...
        while True:
            ...
            if len(ready_set) >= (2 * self.t + 1):
                ...
                if self.tagvars[tag]['all_shares_valid'] and not output:
                    ...
                    self.output_queue.put_nowait(int_shares)
                    output = True
    ...
    def _get_dealer_msg(self, values_dealer, n):
        ...
        for i in range(n):
            ...
            witness = [self.poly_commit.create_witness(phi_hat[k], i + 1) for k in range(secret_count)]
            # print(self.poly_commit.batch_verify_eval(commitments, i + 1, phis_i, witness))
            if not flag:
                z = (phis_i, phi_hats_i, witnesses, aux_info)
            else:
                z = (phis_i, witness)
    ...
    def _handle_dealer_msgs(self, tag, dispersal_msg, rbc_msg):
        ...
        try:
            if not flag[0]:
                ...
                for i in range(self.n):
                    shares[i], witnesses[i] = SymmetricCrypto.decrypt(str(shared_key[i]).encode(), dispersal_msg[i])
        ...
        if all_shares_valid:
            for i in range(self.n):
                if self.poly_commit.batch_verify_eval(commitments[i], self.my_id + 1, shares[i], witnesses[i]):
                    self.tagvars[tag]['shares'][i] = shares[i]
                else:
                    self.tagvars[tag]['implication_index'] = i
                    all_shares_valid = False
        print("hand_over", all_shares_valid)
