...
lib.pyBatchprodverify.argtypes = [c_char_p, c_char_p, c_char_p, c_char_p, c_char_p, c_char_p]

# lib.pyBatchprodverify.argtypes = [c_char_p, c_char_p, c_char_p, c_char_p, c_char_p, c_char_p, c_char_p, c_int]

lib.pyTriplesCompute.argtypes = [c_char_p, c_char_p, c_char_p]
...
class Beaver():
    ...
    #@profile
    def _get_dealer_msg(self, avss_id, values_dealer, msgmode, n):
        ...
        self.tagvars[tag]['comandshares_ab'] = {}
        
        begin_time = time.time()
        
        if msgmode == "beaver_triple":
            serialized_com_proof = values_dealer
            deserialized_commandprooflist = json.loads(serialized_com_proof.decode('utf-8'))
            self.tagvars[tag]['comandshares_ab'] = deserialized_commandprooflist
            commitmentlist = deserialized_commandprooflist["commitment"]
            prooflist = deserialized_commandprooflist["proof"]
            serialized_commitmentlist = json.dumps(commitmentlist).encode('utf-8')
            serialized_prooflist = json.dumps(prooflist).encode('utf-8')
    
            commitmentlistandprooflist = lib.pyParseRandom(self.srs_kzg, serialized_commitmentlist, serialized_prooflist, self.t, self.my_id)
        end_time = time.time()
        print(f"产生证据的时间: {(end_time - begin_time)}")
        deserialized_commitmentlistandprooflist = json.loads(commitmentlistandprooflist.decode('utf-8'))
        commitments = deserialized_commitmentlistandprooflist["commitments_c"]
        zkProof_ab = deserialized_commitmentlistandprooflist["zkProof_ab"]
        zkProof_c_zero = deserialized_commitmentlistandprooflist["zkProof_c_zero"]
        prodProofs = deserialized_commitmentlistandprooflist["prodProofs"]
        
        begin_time = time.time()
        proofandshares = []
        for i in range(self.n):
            proofandshares.append(deserialized_commitmentlistandprooflist["proofs_c"][i])
        ...
        for i in range(n):
            ...
            z = (proofandshares[i], zkProof_ab, zkProof_c_zero, prodProofs)
            dispersal_msg_list[i] = SymmetricCrypto.encrypt(str(shared_keys[i]).encode(), z)
        
        end_time = time.time()
        print(f"计算密钥和加密的时间: {(end_time - begin_time)}")
    ...
    def _handle_dealer_msgs_batch(self, tag, dispersal_msg, rbc_msg, index):
        ...
        zkProof_ab = [None] * self.n
        zkProof_c_zero = [None] * self.n
        prodProofs = [None] * self.n
        ...
        try:
            for i in index:
                ...
                serialized_sharedkey =  lib.pySharedKeysGen_recv(serialized_ephemeralpublickey[i], self.private_key)
                proofandshares[i], zkProof_ab[i], zkProof_c_zero[i], prodProofs[i] = SymmetricCrypto.decrypt(str(serialized_sharedkey).encode(), dispersal_msg[i])
            ...
            all_shares_valid = False
        end_time = time.time()
        print(f"密钥生成的时间: {(end_time - begin_time)}")
        ...
        filtered_zkProof_ab = [item for item in zkProof_ab if item is not None ]
        filtered_zkProof_c_zero = [item for item in zkProof_c_zero if item is not None ]
        filtered_prodProofs = [item for item in prodProofs if item is not None ]
        ...
        serialized_zkProof_ab = json.dumps(filtered_zkProof_ab).encode('utf-8')
        serialized_zkProof_c_zero = json.dumps(filtered_zkProof_c_zero).encode('utf-8')
        serialized_prodProofs = json.dumps(filtered_prodProofs).encode('utf-8')
        
        serialized_commitmentlist_ab = json.dumps(self.tagvars[tag]['comandshares_ab']["commitment"]).encode('utf-8')

        begin_time = time.time()
        print("承诺", lib.pyBatchVerify_all(self.srs_kzg, serialized_commitment, 
                                            serialized_proofandshares, self.my_id))
        end_time = time.time()
        print(f"验证承诺的时间: {(end_time - begin_time)}")
        
        
        begin_time = time.time()
        print("证据", lib.pyBatchprodverify(self.srs_kzg, serialized_commitment, serialized_commitmentlist_ab, 
                                             serialized_zkProof_ab, serialized_zkProof_c_zero, 
        ...
        # begin_time = time.time()
        # if all_shares_valid:
        #     if lib.pyBatchVerify_all(self.srs_kzg, serialized_commitment, 
        #                             serialized_proofandshares, self.my_id) == int(1):
        #         if lib.pyBatchprodverify(self.srs_kzg, serialized_commitment, serialized_commitmentlist_ab, 
        #                                      serialized_zkProof_ab, serialized_zkProof_c_zero, serialized_prodProofs) == int(1):
        #             self.tagvars[tag]['commitments'] = serialized_commitment
        #             self.tagvars[tag]['proofsandshares'] = serialized_proofandshares 
        #     else:
    ...
    async def _process_avss_msg(self, avss_id, client, dealer_id, broadcast_msg, avid_task):
        ...
        shares_ab = self.tagvars[tag]['comandshares_ab']["proof"]
        serialized_shares_ab = json.dumps(shares_ab).encode('utf-8')
        
        serializedcoms_proofs = lib.pyTriplesCompute(serialized_input, serialized_shares_ab, self.tagvars[tag]['proofsandshares'])
        
        end_time = time.time()
        print(f"计算beaver triples的时间: {(end_time - begin_time)}")
    ...
    async def avss(self, avss_id, client, msgmode, values=None, dealer_id=None, client_mode=False):
        ...
        if self.my_id == dealer_id:
            ...
            broadcast_msg, dispersal_msg_list = self._get_dealer_msg(avss_id, values, msgmode, n)
